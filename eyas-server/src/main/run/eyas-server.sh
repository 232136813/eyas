...
# This second pidfile exists for legacy purposes, from the days when eyas
...
case "$1" in
  ...
  stop)
    ...
    curl -m 5 -s http://127.0.0.1:8082/InvokeAction//Eyas%3Aname%3DEyasServer/action=stop?action=stop > /dev/null
    tries=0
    while running; do
      tries=$((tries + 1))
      if [ $tries -ge 15 ]; then
